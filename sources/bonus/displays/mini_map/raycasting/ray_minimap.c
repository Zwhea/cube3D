...
/*   Updated: 2023/10/09 16:41:31 by twang            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ray_minimap(t_game *g, t_minimap *mini)
{
	...
	while (mini->degree >= - M_PI / 6 - M_PI_2)
	{
		mini->plane_y = 0;
		mini->fader = 0;
		while (mini->plane_y <= 165)
		{
			...
			if (my_mlx_pixel_get(&g->draw, mini->throw_x, mini->throw_y) == H_BLACK || \
				my_mlx_pixel_get(&g->draw, mini->throw_x, mini->throw_y) == H_GREY)
				break ;
			if (mini->throw_x >= 0 && mini->throw_y >= 0)
			{
				mini->color = _get_fov_shade(mini->fader);
				my_mlx_pixel_put(&g->draw, (int)mini->throw_x, (int)mini->throw_y, mini->color);
			}
			mini->fader++;
			mini->plane_y++;
		}
