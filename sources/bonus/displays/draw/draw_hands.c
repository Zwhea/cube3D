...
/*   Updated: 2023/10/12 10:51:05 by twang            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_hands(t_game *g)
{
	...
	while (x_text < g->sprites.player.width)
	{
		...
		while (y_text < g->sprites.player.height)
		{
			...
			if (color != H_WHITE && (y_text + (int)(cos(g->player.hand_move) * 10) + 10) < WINDOW_Y)
				my_mlx_pixel_put(&g->draw, x_text, y_text + (int)(cos(g->player.hand_move) * 10) + 10, color);
			y_text++;
		}
		x_text++;
	}
